...
由于在编译时期抽取信息，这里的 controller 是不能进行实例化的，因此我们需要把 controller 以类型参数的方式传递进去，然后抽取它每个 action 方法上面的注解信息(接口描述)以及返回值类型(`SwaAction[A, R]`)，通过返回值类型来进一步抽取请求参数信息和响应参数信息。最终的返回结果应该是一个 `List` 的 JSON 字符串，里面有每个接口的描述信息(Action)。
